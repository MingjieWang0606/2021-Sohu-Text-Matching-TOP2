from model import BertClassifierSingleModel, SBERTSingleModel
...
def infer(model, device, dev_dataloader, test_dataloader, search_thres=True, threshold_fixed_a=0.5, threshold_fixed_b=0.5, save_valid=True):
    ...
    for idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        source_input_ids, target_input_ids, ids, types = batch
        source_input_ids = source_input_ids.to(device)
        target_input_ids = target_input_ids.to(device)
...
if __name__=='__main__':
    ...
    # model = BertClassifierSingleModel(pretrained, hidden_size=config.hidden_size)
    model = SBERTSingleModel(pretrained, hidden_size=config.hidden_size)
    ...
    dev_dataset = SentencePairDatasetForSBERT(dev_data_dir, True, pretrained, clip=clip_method)
    ...
    test_dataset = SentencePairDatasetForSBERT(test_data_dir, False, pretrained, clip=clip_method)
